...
Andrew Ng은 이렇게 말했다 — *"회사 문을 열고 들어갈 때 느끼는 로고의 설렘에서 배우는 게 아닙니다. 매일 함께 부대끼며 일하는 사람들에게서 배웁니다."* 우리가 진짜 신경 쓰는 건 바로 그거다. 브랜드 후광이 아니라, 헤드카운트가 아니라, 서로에게서 배우고 함께 성장할 수 있는 사람들을 모으는 것 — "좋은 동료"의 기준 자체가 바뀌고 있는 지금이니까.

우리는 최전선에서 자리를 잡고 선구자로서 전진하기로 했다. 업계가 표준에 수렴하길 기다리지 않았다. 우리만의 기준을 만들었다.

우리가 먼저 버린 것은 기존의 코딩 테스트였다. 새로운 시험을 설계하면서 네 가지 질문과 마주했다: 기존 테스트는 왜 안 되는가? 공정한 환경을 어떻게 만들 것인가? 문제의 모호함은 어디까지 허용할 것인가? 테스트 가능성과 모호함을 어떻게 양립시킬 것인가?
...
graph TD
    ...
    D -->|하지만...| E["힌트를 줄수록 AI의 일이 쉬워짐<br/>후보자의 사고는 얕아짐"]
    E -.->|원점| A
...
graph BT
    ...
    DT["🦆 덕 타이핑<br/>외부 행동이 곧 검증"]
    BDT["🔍 덕 타이핑 너머<br/>내부 이해가 성장 가능성을 보여줌"]
...
한 가지만 미리 공개하자면 — 결과가 직관과 달랐다. 사고의 깊이가 기능적 완성도보다 중요할 거라고 설계했다. 품질 평가가 결정적 변별 요소가 될 거라고 기대했다. 그런데 실제로 후보자들을 오프라인에서 만나고 나니, 동작하는 결과물에 집중한 후보자들 — 품질 점수가 낮아 "Ace"가 아닌 "Craftsman" 등급을 받은 — 이 면접관들로부터 더 강한 추천을 받았다. 이것이 무엇을 의미하는지는 아직 생각 중이다. 다음 글에서 이어가겠다.
